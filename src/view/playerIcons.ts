...
export const drawYoutubeIcon = (x: number, y: number, color: string) =>
  drawAt(x, y, 512, 12, search, color);

export const drawPlayIcon = (x: number, y: number) =>
  drawAt(x, y, regularWidth, 20, play, sp.selectedCircle);

export const drawPauseIcon = (x: number, y: number) =>
  drawAt(x, y, regularWidth, 20, pause, sp.selectedCircle);
